...
class T5FineTuner(pl.LightningModule):
    ...
    def train_dataloader(self):
        train_dataset = get_dataset(
            tokenizer=self.tokenizer, type_path="boolq_train", args=self.hparams)
    ...
    def val_dataloader(self):
        val_dataset = get_dataset(
            tokenizer=self.tokenizer, type_path="boolq_val", args=self.hparams)
...
class BooleanDataset(Dataset):
    ...
    def _build(self):
        for idx in range(len(self.data)):
            passage, true_false, target = self.data.loc[idx, self.passage_column], self.data.loc[idx,
                                                                                                 self.true_false], self.data.loc[idx, self.target_column]
...
args_dict.update({'data_dir': 'boolq_data', 'output_dir': 't5_boolq',
                 'num_train_epochs': 4, 'max_seq_length': 256})
